...
def collide(obj1, obj2):
    ...
    return obj1.mask.overlap(obj2.mask, (offset_x,offset_y))!= None #(else return (x,y))
...
def main():
    ...
    while run:
        ...
        #It allows you to detect more than one button at a time
        #Keys actually return the dict
        ...
        if keys[pygame.K_w] and player.y - player_vel > 0:
            player.y -= player_vel
        if keys[pygame.K_s] and player.y + player_vel +15 < HEIGHT - player.get_heigth():
            player.y += player_vel
        if keys[pygame.K_SPACE]:
            player.shoot()
        if keys[pygame.K_LEFT] and player2.x - player_vel>0:
            player2.x -=player_vel
        if keys[pygame.K_RIGHT] and player2.x + player_vel < WIDTH - player2.get_width():
            player2.x += player_vel
        if keys[pygame.K_UP] and player2.y - player_vel > 0:
            player2.y -= player_vel
        if keys[pygame.K_DOWN] and player2.y + player_vel +15 < HEIGHT - player2.get_heigth():
            player2.y += player_vel
        if keys[pygame.K_0]:
            player2.shoot()
